...
func validateArrayField(item map[string]interface{}, field models.Field) error {
	arrayItems, ok := validationArrayItems(item[field.Name])
	if !ok {
...
func validationArrayItems(value interface{}) ([]interface{}, bool) {
...
func validateFieldBase(item map[string]interface{}, field models.Field) error {
	...
	case "array":
		...
		_, ok := validationArrayItems(fieldValue)
		if !ok {
	...
	case "enum":
		...
		if !ok {
			...
			if list, ok := rules["enum"].([]interface{}); ok {
				allowed = enumStrings(list)
			}
		}
		if len(allowed) == 0 && len(field.EnumList) > 0 {
			allowed = enumStrings(field.EnumList)
		}
		found := false
