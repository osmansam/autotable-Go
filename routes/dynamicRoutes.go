...
func DynamicRoutes(baseUrl string, app *fiber.App) {
	...
	dynamicGroup.Get("/table-source", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("GetAllDynamicModelItemsWithPagination"), middlewares.GeneralRateLimit(), middlewares.SearchRateLimit(), controllers.GetTableSource)
