...
func DynamicRoutes(baseUrl string, app *fiber.App) {
	...
	dynamicGroup.Post("/", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("CreateDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.CreateDynamicModelItem)
	dynamicGroup.Post("/multiple", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("CreateMultipleDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.BulkRateLimit(), controllers.CreateMultipleDynamicModelItem)
	dynamicGroup.Post("/export", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("ExportDynamicModelItems"), middlewares.GeneralRateLimit(), middlewares.ExportRateLimit(), controllers.ExportDynamicModelItems)
	...
	dynamicGroup.Get("/testPipeline", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("TestPipeline"), middlewares.GeneralRateLimit(), middlewares.SearchRateLimit(), controllers.TestPipeline)
	...
	dynamicGroup.Get("/api", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("ExecuteDynamicAPI"), middlewares.GeneralRateLimit(), middlewares.ExecuteRateLimit(), controllers.ExecuteDynamicAPI)
	dynamicGroup.Patch("/multiple", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("UpdateMultipleDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.BulkRateLimit(), controllers.UpdateMultipleDynamicModelItem)
	dynamicGroup.Delete("/multiple", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("DeleteMultipleDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.BulkRateLimit(), controllers.DeleteMultipleDynamicModelItem)
	...
	dynamicGroup.Patch("/:id", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("UpdateDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.UpdateDynamicModelItem)
