...
func DynamicRoutes(baseUrl string, app *fiber.App) {
	...
	dynamicGroup.Get("/table-source", middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("GetTableSource"), middlewares.GeneralRateLimit(), middlewares.SearchRateLimit(), controllers.GetTableSource)
	...
	dynamicGroup.Post("/workflow/:workflowName", middlewares.DefaultBodySizeLimit(), middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("ExecuteWorkflow"), middlewares.GeneralRateLimit(), middlewares.ExecuteRateLimit(), controllers.ExecuteWorkflow)
	...
	dynamicGroup.Post("/:schema/:id/array/:field", middlewares.DefaultBodySizeLimit(), middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("UpdateDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.AddDynamicArrayRow)
	dynamicGroup.Patch("/:schema/:id/array/:field/reorder", middlewares.DefaultBodySizeLimit(), middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("UpdateDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.ReorderDynamicArrayRows)
	dynamicGroup.Patch("/:schema/:id/array/:field/:rowIdentity", middlewares.DefaultBodySizeLimit(), middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("UpdateDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.UpdateDynamicArrayRow)
	dynamicGroup.Delete("/:schema/:id/array/:field/:rowIdentity", middlewares.DefaultBodySizeLimit(), middlewares.PublicRateLimit(), middlewares.ConditionalAuthentication("DeleteDynamicModelItem"), middlewares.GeneralRateLimit(), middlewares.WriteRateLimit(), controllers.DeleteDynamicArrayRow)
